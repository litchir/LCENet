...
def main(args):
    ...
    experiment_dir = 'log/eval/' + args.log_dir
    ...
    classifier = MODEL.Network(num_part, in_ca, mlps).cuda()
    ...
    with torch.no_grad():
        ...
        for batch_id, (inputs, label, target) in tqdm(enumerate(testDataLoader), total=len(testDataLoader),
                                                      smoothing=0.9):
            batchsize, num_point, _ = inputs.size()
            cur_batch_size, NUM_POINT, _ = inputs.size()
            inputs, label, target = inputs.float().cuda(), label.long().cuda(), target.long().cuda()
            ...
            for _ in range(args.num_votes):
                seg_pred, _ = classifier(points, feature)
                vote_pool += seg_pred
